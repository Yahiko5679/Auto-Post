...
def _build_card(
    ...
    # ── Extract all metadata safely ──────────────────────────────────────────
    title    = meta.get("title", "Unknown Title").upper()
    year     = str(meta.get("year", "") or meta.get("release_date", "")[:4])
    rating   = meta.get("imdb_rating") or meta.get("rating") or meta.get("score", "")
    status   = meta.get("status", "")
    episodes = str(meta.get("episodes", "") or meta.get("total_episodes", ""))
    seasons  = str(meta.get("seasons", "") or meta.get("season_count", ""))
    genres   = meta.get("genres", "")
    overview = meta.get("overview") or meta.get("synopsis", "") or meta.get("description", "")
